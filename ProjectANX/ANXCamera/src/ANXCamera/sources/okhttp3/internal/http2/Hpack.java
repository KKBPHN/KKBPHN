...
final class Hpack {
    ...
    final class Reader {
        ...
        private ByteString getName(int i) {
            return (isStaticHeader(i) ? Hpack.STATIC_HEADER_TABLE[i] : this.dynamicTable[dynamicTableIndex(i - Hpack.STATIC_HEADER_TABLE.length)]).name;
        }
        ...
        public void readHeaders() {
            while (!this.source.exhausted()) {
                ...
                } else if ((readByte & Byte.MIN_VALUE) == Byte.MIN_VALUE) {
                ...
                if (num != null) {
                    ...
                    if (i2 > 1 && i2 < 8) {
                        if (!Util.equal(Hpack.STATIC_HEADER_TABLE[i2 - 1].value, byteString)) {
                            if (Util.equal(Hpack.STATIC_HEADER_TABLE[i2].value, byteString)) {
                                i = i2;
                                i2++;
                                if (i2 == -1) {
                                    ...
                                    while (true) {
                                        ...
                                        if (Util.equal(this.dynamicTable[i6].name, asciiLowercase)) {
                                            if (Util.equal(this.dynamicTable[i6].value, byteString)) {
                                                ...
                                                i = (i6 - this.nextHeaderIndex) + Hpack.STATIC_HEADER_TABLE.length;
                                            }
                                        }
                                        i6++;
                                    }
