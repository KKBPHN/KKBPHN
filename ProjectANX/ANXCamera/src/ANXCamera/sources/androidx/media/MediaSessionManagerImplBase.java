...
class MediaSessionManagerImplBase implements MediaSessionManagerImpl {
    ...
    private boolean isPermissionGranted(RemoteUserInfoImpl remoteUserInfoImpl, String str) {
        ...
        if (pid < 0) {
            if (context.getPackageManager().checkPermission(str, remoteUserInfoImpl.getPackageName()) != 0) {
                z = false;
            }
            return z;
        }
        if (context.checkPermission(str, remoteUserInfoImpl.getPid(), remoteUserInfoImpl.getUid()) != 0) {
            z = false;
        }
        return z;
    }
