...
class AppCompatTextViewAutoSizeHelper {
    ...
    private static Object accessAndReturnWithDefault(@NonNull Object obj, @NonNull String str, @NonNull Object obj2) {
        try {
            Field textViewField = getTextViewField(str);
            return textViewField == null ? obj2 : textViewField.get(obj);
    ...
    private StaticLayout createStaticLayoutForMeasuringPre16(CharSequence charSequence, Alignment alignment, int i) {
        float floatValue = ((Float) accessAndReturnWithDefault(this.mTextView, "mSpacingMult", Float.valueOf(1.0f))).floatValue();
        float floatValue2 = ((Float) accessAndReturnWithDefault(this.mTextView, "mSpacingAdd", Float.valueOf(0.0f))).floatValue();
        boolean booleanValue = ((Boolean) accessAndReturnWithDefault(this.mTextView, "mIncludePad", Boolean.valueOf(true))).booleanValue();
    ...
    private static Object invokeAndReturnWithDefault(@NonNull Object obj, @NonNull String str, @NonNull Object obj2) {
        try {
    ...
    private boolean suggestedSizeFitsInSpace(int i, RectF rectF) {
        ...
        initTempTextPaint(i);
        StaticLayout createLayout = createLayout(text, (Alignment) invokeAndReturnWithDefault(this.mTextView, "getLayoutAlignment", Alignment.ALIGN_NORMAL), Math.round(rectF.right), maxLines);
        ...
        if (isAutoSizeEnabled()) {
            if (this.mNeedsAutoSizeText) {
                if (this.mTextView.getMeasuredHeight() > 0 && this.mTextView.getMeasuredWidth() > 0) {
                    int measuredWidth = VERSION.SDK_INT >= 29 ? this.mTextView.isHorizontallyScrollable() : ((Boolean) invokeAndReturnWithDefault(this.mTextView, "getHorizontallyScrolling", Boolean.valueOf(false))).booleanValue() ? 1048576 : (this.mTextView.getMeasuredWidth() - this.mTextView.getTotalPaddingLeft()) - this.mTextView.getTotalPaddingRight();
    ...
    /* access modifiers changed from: 0000 */
    @VisibleForTesting
    public StaticLayout createLayout(CharSequence charSequence, Alignment alignment, int i, int i2) {
        int i3 = VERSION.SDK_INT;
        return i3 >= 23 ? createStaticLayoutForMeasuring(charSequence, alignment, i, i2) : i3 >= 16 ? createStaticLayoutForMeasuringPre23(charSequence, alignment, i) : createStaticLayoutForMeasuringPre16(charSequence, alignment, i);
    }
