...
public class HttpUrlFetcher implements DataFetcher {
    ...
    private InputStream getStreamForSuccessfulRequest(HttpURLConnection httpURLConnection) {
        ...
        if (TextUtils.isEmpty(httpURLConnection.getContentEncoding())) {
            inputStream = ContentLengthInputStream.obtain(httpURLConnection.getInputStream(), (long) httpURLConnection.getContentLength());
        } else {
            String str = TAG;
    ...
    private InputStream loadDataWithRedirects(URL url, int i, URL url2, Map map) {
        if (i < 5) {
            if (url2 != null) {
                try {
