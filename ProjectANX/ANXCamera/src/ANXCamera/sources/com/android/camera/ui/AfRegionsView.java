...
public class AfRegionsView extends View implements Rotatable {
    ...
    private void prepareMatrix() {
        ...
        if (i3 == 90 || i3 == 270) {
            i2 = this.mActiveArraySize.height() * renderHeight;
            i = this.mActiveArraySize.width();
        } else {
            i2 = this.mActiveArraySize.width() * renderHeight;
            i = this.mActiveArraySize.height();
        }
        Util.prepareMatrix(this.mMatrix, false, this.mCameraDisplayOrientation, i2 / i, renderHeight, getWidth() / 2, getHeight() / 2, this.mActiveArraySize.width(), this.mActiveArraySize.height());
        if (!this.mLightingOn) {
