...
public class YuvToRgbRender extends ShaderRender {
    ...
    public boolean draw(DrawAttribute drawAttribute) {
        ...
        if (drawAttribute.getTarget() != 11) {
            ...
            Image image = drawYuvAttribute.mImage;
            if (image != null) {
                image.getTimestamp();
            }
            if (drawYuvAttribute.mYuvImage != null) {
                genMiYuvTextures(drawYuvAttribute);
                Log.e(TAG, "yuv image is not available!");
            ...
            if (drawYuvAttribute.mBlockWidth == 0 && drawYuvAttribute.mBlockHeight == 0) {
                iArr = this.mYuvTextureIds;
                f3 = 0.0f;
                f2 = 0.0f;
                f = (float) drawYuvAttribute.mPictureSize.getWidth();
                i = drawYuvAttribute.mPictureSize.getHeight();
            } else {
                iArr = this.mYuvTextureIds;
                f3 = 0.0f;
                f2 = 0.0f;
                f = (float) drawYuvAttribute.mBlockWidth;
                i = drawYuvAttribute.mBlockHeight;
            }
            drawTexture(iArr, f3, f2, f, (float) i);
    ...
    public void initShaderValue() {
        ...
        if (C0124O00000oO.isMTKPlatform() || 2 == xiaomiYuvFormat) {
