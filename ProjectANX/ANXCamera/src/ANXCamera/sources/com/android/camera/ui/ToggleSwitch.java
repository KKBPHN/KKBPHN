...
public class ToggleSwitch extends CompoundButton implements Checkable {
    ...
    private float[] compThumbRoundRectAttr(float f) {
        ...
        float f3 = (((float) (i + i3)) * (1.0f - f)) + (((((float) i) / 2.0f) + f2) * f);
        float f4 = (float) (i + i3);
        float f5 = (((float) i2) + f4) - ((float) (i3 * 2));
        return new float[]{f3, f4, ((f2 - ((float) i3)) - ((((float) i) * 3.0f) / 2.0f)) + f3, f5, (f5 - f4) * 0.5f};
    }
    ...
    public void setChecked(boolean z) {
        if (!this.isMoving && this.mChecked != z) {
            ...
            if (!this.mBroadcasting) {
                ...
                this.state = this.mChecked ? State.SWITCHING_ANIM_ON : State.SWITCHING_ANIM_OFF;
