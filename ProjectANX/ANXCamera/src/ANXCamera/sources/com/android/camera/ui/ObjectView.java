...
public class ObjectView extends FrameView {
    ...
    private void detectMovingStyle(float f, float f2, float f3, float f4) {
        ...
        if (f3 < 0.0f || f4 < 0.0f) {
            boolean z2 = false;
            this.mMovingMode = 0;
            float abs3 = Math.abs(f - this.mObjectRect.left);
            float abs4 = Math.abs(f - this.mObjectRect.right);
            if (abs3 > ((float) TOUCH_TOLERANCE) || abs3 >= abs4) {
                if (abs4 <= ((float) TOUCH_TOLERANCE)) {
                    i3 = this.mMovingMode | 4;
                }
                abs = Math.abs(f2 - this.mObjectRect.top);
                abs2 = Math.abs(f2 - this.mObjectRect.bottom);
                z = abs > ((float) TOUCH_TOLERANCE);
                if (abs < abs2) {
                    z2 = true;
                }
                if (!z2 || !z) {
                    i2 = this.mMovingMode | 2;
                } else {
                    if (abs2 <= ((float) TOUCH_TOLERANCE)) {
                        i2 = this.mMovingMode | 8;
                    }
                ...
                i3 = this.mMovingMode | 1;
            }
            this.mMovingMode = i3;
            abs = Math.abs(f2 - this.mObjectRect.top);
            abs2 = Math.abs(f2 - this.mObjectRect.bottom);
            if (abs > ((float) TOUCH_TOLERANCE)) {
            }
            if (abs < abs2) {
            }
            if (!z2 || !z) {
            ...
            if (Math.abs(this.mObjectRect.left - Math.min(f, f3)) <= ((float) VERTEXS_TOUCH_TOLERANCE) && Math.abs(this.mObjectRect.right - Math.max(f, f3)) <= ((float) VERTEXS_TOUCH_TOLERANCE) && Math.abs(this.mObjectRect.top - Math.min(f2, f4)) <= ((float) VERTEXS_TOUCH_TOLERANCE) && Math.abs(this.mObjectRect.bottom - Math.max(f2, f4)) <= ((float) VERTEXS_TOUCH_TOLERANCE)) {
                i = 32;
            }
            invalidate();
        }
    ...
    private RectF mapRect(RectF rectF, boolean z) {
        ...
        rectF2.set(rectF);
        (z ? this.mMatrix : this.mFace2UIMatrix).mapRect(rectF2);
        return rectF2;
    }
    ...
    public boolean initializeTrackView(RectF rectF, boolean z) {
        if (rectF != null) {
            MistatsWrapper.commonKeyTriggerEvent(FeatureName.VALUE_T2T_TIMES, null, null);
            ...
            if (i == 3) {
                ...
                if (cameraHardwareFace.score < 1) {
                    if (this.mLostTrackThreshold == 0) {
                        ...
                        int i2 = NEAR_EDGE;
                        int i3 = (f2 < ((float) i2) || rectF.top - rectF2.top < ((float) i2) || rectF2.right - rectF.right < ((float) i2) || rectF2.bottom - rectF.bottom < ((float) i2)) ? 150 : 50;
                        this.mLostTrackThreshold = i3;
                    }
                    this.mLostTrackingNum++;
                    int i4 = this.mLostTrackingNum;
                    if (i4 % 5 == 0 || i4 == this.mLostTrackThreshold) {
                ...
                boolean z2 = ((double) cameraHardwareFace.rect.width()) / ((double) this.mPreviewWidth) > 0.33d || ((double) cameraHardwareFace.rect.height()) / ((double) this.mPreviewHeight) > 0.33d;
                this.mIsBigEnoughRect = z2;
