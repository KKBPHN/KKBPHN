...
public class MediaAudioEncoder extends MediaEncoder {
    ...
    class AudioEffectThread extends Thread {
        private short[] DUMMY_SAMPLE = new short[0];
        private BlockingQueue mBufferQueue = new LinkedBlockingQueue();
        ...
        private void processBuffer() {
            short[] sArr = (short[]) this.mBufferQueue.take();
            if (sArr != this.DUMMY_SAMPLE) {
                SoundEffect soundEffect = this.mSoundEffect;
                if (soundEffect != null) {
                    soundEffect.putSamples(sArr);
                } else {
                    encodeSamples(sArr);
                }
            }
