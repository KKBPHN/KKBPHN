...
public class GradienterEffectRender extends PixelEffectRender {
    ...
    private void filteRotation(float f) {
        ...
        if (f4 != -1.0f) {
            ...
            f3 = (this.mLastRotation * 0.7f) + (0.3f * f);
        } else {
            this.mLastRotation = f;
            f2 = this.mLastRotation;
            if (f2 < 360.0f) {
                f3 = f2 - 360.0f;
            }
        ...
        if (f2 < 360.0f) {
            ...
            sb2.append("filteRotation deviceRotation=");
            sb2.append(f);
            sb2.append(" mLastRotation=");
            sb2.append(this.mLastRotation);
            Log.v(TAG, sb2.toString());
        }
        StringBuilder sb22 = new StringBuilder();
        sb22.append("filteRotation deviceRotation=");
        sb22.append(f);
        sb22.append(" mLastRotation=");
        sb22.append(this.mLastRotation);
    ...
    private float getRotation() {
        ...
        float orientation = this.mLastRotation - ((float) EffectController.getInstance().getOrientation());
        if (orientation > 180.0f) {
            orientation -= 360.0f;
        }
        boolean z = Math.abs(orientation) < 0.5f;
        if (z != this.mKeepZero) {
            this.mKeepZero = z;
            this.mShiftTimes = 1;
        } else {
            int i = this.mShiftTimes;
            if (i < 5) {
                this.mShiftTimes = i + 1;
            }
    ...
    public void initShaderValue(boolean z) {
        ...
        if (ModuleManager.isSquareModule()) {
            ...
            f2 -= (float) ((this.mSquareModeExtraMargin * this.mPreviewWidth) / Display.getWindowWidth());
        } else {
