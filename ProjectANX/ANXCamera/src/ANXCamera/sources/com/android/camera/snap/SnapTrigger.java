...
public class SnapTrigger implements SnapStatusListener {
    ...
    public boolean shouldQuitSnap() {
        if (!ProximitySensorLock.enabled() || !Util.isNonUIEnabled()) {
            ProximitySensorLock proximitySensorLock = this.mProximityLock;
            boolean z = proximitySensorLock != null && proximitySensorLock.shouldQuitSnap();
            return z;
        }
        ...
        if (isNonUI) {
            CameraStatUtils.trackPocketModeEnter(NonUI.POCKET_MODE_NONUI_ENTER_SNAP);
        }
        return isNonUI;
    }
    ...
    private void vibrator(long[] jArr) {
        try {
            ...
            Log.e(TAG, "vibrator exception", (Throwable) e);
        }
    }
