...
public class PagerGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
    ...
    private SparseArray mItemFrames = new SparseArray();
    ...
    private void addOrRemove(Recycler recycler, Rect rect, int i) {
    ...
    private Rect getItemFrameByPosition(int i) {
        int i2;
        Rect rect = (Rect) this.mItemFrames.get(i);
        if (rect == null) {
            rect = new Rect();
    ...
    private void recycleAndFillItems(Recycler recycler, State state, boolean z) {
    ...
    private void setPageIndex(int i, boolean z) {
        if (i != this.mLastPageIndex) {
            ...
            if ((!z || this.mChangeSelectInScrolling) && i >= 0) {
                PageListener pageListener = this.mPageListener;
    ...
    public void onMeasure(Recycler recycler, State state, int i, int i2) {
        ...
        setMeasuredDimension(MeasureSpec.makeMeasureSpec(size, mode), MeasureSpec.makeMeasureSpec(size2, mode2));
    }
    ...
    public void smoothScrollToPage(int i) {
        ...
        if (i < 0 || i >= this.mLastPageCount) {
            String str = TAG;
            StringBuilder sb = new StringBuilder();
            sb.append("pageIndex is outOfIndex, must in [0, ");
            sb.append(this.mLastPageCount);
            sb.append(").");
            Log.e(str, sb.toString());
        } else if (this.mRecyclerView == null) {
