    ...
    public void onCreate() {
        ...
        CameraSchedulers.sCameraSetupScheduler.scheduleDirect(new Runnable() {
            public void run() {
                ...
                if (C0122O00000o.instance().OOo0OO() && WatermarkMiSysUtils.isFileExist(Util.WATERMARK_FRONT_FILE_NAME) == 0) {
                    Util.generateFrontWatermark2File();
                }
                if (WatermarkMiSysUtils.isFileExist(Util.WATERMARK_CINEMATIC_RATIO_FILE_NAME) == 0) {
                    Util.generateCinematicRatioWatermark2File();
                }
            }
        });
