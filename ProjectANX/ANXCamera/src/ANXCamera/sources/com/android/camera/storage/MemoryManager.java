...
public class MemoryManager implements StorageListener {
    ...
    private void initLimit() {
        float baseMemory = (float) (this.mMaxMemory - getBaseMemory());
        if (Storage.isUsePhoneStorage()) {
            this.mSaveTaskMemoryLimit = (int) (baseMemory * 0.6f);
        } else {
            this.mSaveTaskMemoryLimit = (int) (baseMemory * 0.5f);
    ...
    public boolean isNeedSlowDown() {
        boolean z = true;
        if (!C0124O00000oO.isMTKPlatform() ? this.mSaverMemoryUse < this.mSaveTaskMemoryLimit / 2 : this.mSaverMemoryUse < (this.mSaveTaskMemoryLimit * 3) / 4) {
            z = false;
        }
        StringBuilder sb = new StringBuilder();
