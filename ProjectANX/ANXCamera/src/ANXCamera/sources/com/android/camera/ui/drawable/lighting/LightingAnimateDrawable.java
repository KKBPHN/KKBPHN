...
public class LightingAnimateDrawable extends Drawable {
    ...
    private void animateMiddleChange(int i) {
        LightingPaintCircle lightingPaintCircle;
        float f;
        float f2;
        float f3;
        cancelAllAnimation();
        if (i == 90 || i == 270) {
            ...
            lightingPaintCircle = this.mPaintCircle;
            f3 = this.mMiddleX;
            f2 = this.mMiddleHorizontalY;
            f = (((((float) Math.min(this.mWidth, this.mHeight)) * this.mCircleRatio) * 1.33f) / 2.0f) / 1.15f;
        } else {
            ...
            lightingPaintCircle = this.mPaintCircle;
            f3 = this.mMiddleX;
            f2 = this.mMiddleVerticalY;
            f = ((((float) Math.min(this.mWidth, this.mHeight)) * this.mCircleRatio) * 1.33f) / 2.0f;
        }
        lightingPaintCircle.setTargetMiddle(f3, f2, f);
    ...
    public void setWidthHeight(int i, int i2, int i3) {
        ...
        if (DataRepository.dataItemGlobal().getDisplayMode() == 1) {
            f = (float) Math.min(this.mWidth, this.mHeight);
        } else {
            f = (float) Math.min(this.mWidth, this.mHeight);
            f3 = 1.77f;
        }
        this.mMiddleHorizontalY = (f * f3) / 2.0f;
        if (CameraSettings.isGifOn()) {
            ...
            float f5 = this.mCircleHeightRatio;
            this.mMiddleVerticalY = f4 * f5;
            this.mMiddleHorizontalY *= f5;
        }
        if (DataRepository.dataItemRunning().getUiStyle() == 3) {
            float f6 = this.mMiddleVerticalY;
            int i4 = this.mTopMargin;
            this.mMiddleVerticalY = f6 + ((float) i4);
    ...
    public void updateMiddleValue(int i, boolean z) {
        ...
        if (!this.isShown || !z) {
            if (i == 90 || i == 270) {
                ...
                lightingPaintCircle = this.mPaintCircle;
                f = this.mMiddleX;
                f3 = this.mMiddleHorizontalY;
                f2 = (((((float) Math.min(this.mWidth, this.mHeight)) * this.mCircleRatio) * 1.33f) / 2.0f) / 1.15f;
            } else {
                ...
                lightingPaintCircle = this.mPaintCircle;
                f = this.mMiddleX;
                f3 = this.mMiddleVerticalY;
                f2 = ((((float) Math.min(this.mWidth, this.mHeight)) * this.mCircleRatio) * 1.33f) / 2.0f;
            }
            lightingPaintCircle.setMiddle(f, f3, f2);
            return;
        }
