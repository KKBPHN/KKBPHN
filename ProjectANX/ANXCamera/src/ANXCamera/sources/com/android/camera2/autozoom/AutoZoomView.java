...
public class AutoZoomView extends View implements IObjectView, OnTouchListener {
    ...
    private RectF fillBoundsInOverlay(int i, float[] fArr) {
        ...
        float f = fArr[i2];
        float f2 = fArr[i2 + 1];
        RectF rectF = new RectF(f / ((float) size.getWidth()), f2 / ((float) this.mPreviewSize.getHeight()), (f + fArr[i2 + 2]) / ((float) this.mPreviewSize.getWidth()), (f2 + fArr[i3]) / ((float) this.mPreviewSize.getHeight()));
