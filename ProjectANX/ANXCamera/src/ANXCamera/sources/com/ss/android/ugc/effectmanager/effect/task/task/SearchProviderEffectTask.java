...
public class SearchProviderEffectTask extends NormalTask {
    ...
    private EffectRequest buildEffectListRequest() {
        ...
        if (!TextUtils.isEmpty(this.mSearchWord)) {
            hashMap.put(EffectConfiguration.KEY_SEARCH_WORD, this.mSearchWord);
        }
        ...
        sb.append(EffectConstants.ROUTE_SEARCH_PROVIDER_LIST);
    ...
    public void execute() {
        ProviderEffectTaskResult providerEffectTaskResult;
                ...
                try {
                    if (!isCanceled()) {
                        ...
                        if (providerEffectListResponse.checkValue()) {
                            providerEffectTaskResult = new ProviderEffectTaskResult(providerEffectListResponse.getData(), null);
                            break;
                        } else if (this.mCurCnt == 0) {
                            ExceptionResult exceptionResult = new ExceptionResult(10002);
                            exceptionResult.setTrackParams(this.mRequestedUrl, this.mSelectedHost, this.mRemoteIp);
                            providerEffectTaskResult = new ProviderEffectTaskResult(new ProviderEffectModel(), exceptionResult);
                            break;
                        }
                    } else {
                        ExceptionResult exceptionResult2 = new ExceptionResult(10001);
                        exceptionResult2.setTrackParams(this.mRequestedUrl, this.mSelectedHost, this.mRemoteIp);
                        providerEffectTaskResult = new ProviderEffectTaskResult(new ProviderEffectModel(), exceptionResult2);
                        break;
                    }
                } catch (Exception e) {
