...
public class UpdateTagRepository implements IHandler {
    ...
    public void checkedTagInHashMap(String str, String str2, IIsTagNeedUpdatedListener iIsTagNeedUpdatedListener) {
        HashMap hashMap = this.mTagsCachedMap;
        if (hashMap == null) {
            iIsTagNeedUpdatedListener.onTagNeedUpdate();
            return;
        }
        if (!hashMap.containsKey(str) || ValueConvertUtil.ConvertStringToLong(str2, -1) > ValueConvertUtil.ConvertStringToLong((String) this.mTagsCachedMap.get(str), -1)) {
            iIsTagNeedUpdatedListener.onTagNeedUpdate();
        } else {
