...
public class AutoInstallFileProvider extends ContentProvider {
    ...
    class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                r6 = file;
                r6 = file.getCanonicalPath();
                r6 = r6;
                Entry entry = null;
                for (Entry entry2 : this.mRoots.entrySet()) {
                    String path = ((File) entry2.getValue()).getPath();
                    if (r6.startsWith(path) && (entry == null || path.length() > ((File) entry.getValue()).getPath().length())) {
                        entry = entry2;
                    }
                }
                if (entry != null) {
                    ...
                    String substring = r6.substring(length);
                    StringBuilder sb = new StringBuilder();
