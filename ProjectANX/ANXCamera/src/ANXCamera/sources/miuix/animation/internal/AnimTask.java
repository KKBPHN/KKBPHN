...
public class AnimTask {
    ...
    private boolean animReachMinimumDuration(AnimRunningInfo animRunningInfo, long j) {
        ...
        return runningTime >= minDuration || minDuration - runningTime < j;
    }
    ...
    /* JADX WARNING: Incorrect type for immutable var: ssa=java.util.Collection, code=java.util.Collection<miuix.animation.internal.AnimRunningInfo>, for r8v0, types: [java.util.Collection, java.util.Collection<miuix.animation.internal.AnimRunningInfo>] */
    ...
    private boolean isNotIn(FloatProperty floatProperty, FloatProperty... floatPropertyArr) {
        ...
        for (int i = 0; i < length; i++) {
            FloatProperty floatProperty2 = floatPropertyArr[i];
            if (floatProperty.equals(floatProperty2) || isSameTranslation(floatProperty, floatProperty2)) {
                return false;
            }
        }
        return true;
    ...
    private boolean isSameTranslation(FloatProperty floatProperty, FloatProperty floatProperty2) {
        return (floatProperty == ViewProperty.TRANSLATION_X && floatProperty2 == ViewProperty.X) || (floatProperty == ViewProperty.X && floatProperty2 == ViewProperty.TRANSLATION_X) || ((floatProperty == ViewProperty.TRANSLATION_Y && floatProperty2 == ViewProperty.Y) || ((floatProperty == ViewProperty.Y && floatProperty2 == ViewProperty.TRANSLATION_Y) || ((floatProperty == ViewProperty.TRANSLATION_Z && floatProperty2 == ViewProperty.Z) || (floatProperty == ViewProperty.Z && floatProperty2 == ViewProperty.TRANSLATION_Z))));
    }

    /* JADX WARNING: Incorrect type for immutable var: ssa=java.util.Collection, code=java.util.Collection<miuix.animation.internal.AnimRunningInfo>, for r2v0, types: [java.util.Collection, java.util.Collection<miuix.animation.internal.AnimRunningInfo>] */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    ...
    private void notifyUpdateAndEnd(Collection collection) {
        ...
        for (Entry entry : this.mUpdateMap.entrySet()) {
            ...
            if (!list2.isEmpty()) {
                notifyForUpdateAndEnd(this.mNotifier, key, list2);
                if (!checkAndNotifyEnd(key, false)) {
                }
            }
            list.add(key);
        }
        removeUpdateInfoOfStoppedTrans(list);
        for (Object remove : list) {
            ObjectPool.release((List) this.mUpdateMap.remove(remove));
        }
        ObjectPool.release(list);
    }
    ...
    /* JADX WARNING: Incorrect type for immutable var: ssa=java.util.List, code=java.util.List<miuix.animation.internal.AnimRunningInfo>, for r5v0, types: [java.util.List, java.util.List<miuix.animation.internal.AnimRunningInfo>] */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private void updateAndStop(List<AnimRunningInfo> list, long j, long j2) {
        for (AnimRunningInfo animRunningInfo : list) {
            if (animRunningInfo.isRunning()) {
                animRunningInfo.run(j2);
                boolean isAnimEnd = isAnimEnd(animRunningInfo, j);
                if (isAnimEnd || (animRunningInfo.status == 2 && animReachMinimumDuration(animRunningInfo, j2))) {
                    boolean z = isAnimEnd && !animRunningInfo.updateToDate().isCanceled;
                    animRunningInfo.stop(z);
                }
            } else if (animRunningInfo.status != 3) {
            }
            this.mRunningList.remove(animRunningInfo);
        }
    }
